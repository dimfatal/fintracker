package tcs4sclient.model.domain.user
...
object AccountsDecoder {
  ...
  implicit val accountsDecoder: Decoder[Accounts]        = deriveDecoder[Accounts]
  ...
  implicit val accountTypesDecoder: Decoder[AccountType] = (cursor: HCursor) =>
    for {
      ...
      result <- value match {
                  ...
                  case s            => DecodingFailure(s"Invalid account type ${s}", cursor.history).asLeft
                }
    } yield result
